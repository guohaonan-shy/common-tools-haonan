...
#### 如何科学地决定建立哪些索引？
核心原则：索引是为查询服务的，不是为表服务的。你应该从查询出发，反推需要什么样的索引来高效支持它。
#### 📊 索引设计决策树

- ✅ **首先，明确业务场景和高频查询，是否为高频查询/排序/连接字段？**
    - 是 →
        - ✅ **是否是多个字段组合经常一起使用？**
            - 是 → 建立 **联合索引（按最左前缀）**
                - 👉 示例：`CREATE INDEX idx_a_b_c ON table(a, b, c);`
            - 否 →
                - ✅ 是否字段出现在 `SELECT` 中？
                    - 是 → 建立 **覆盖索引**
...
- ✅ **WHERE 子句中是否使用函数/表达式？**
    - 是 → ⚠️ 索引 **可能失效**，建议 **改写 SQL**
        - 示例：避免 `WHERE DATE(create_time) = '2024-01-01'`
    - 否 → 索引可以有效命中

---

- ✅ **是否涉及高并发写入（INSERT/UPDATE/DELETE）？**
    - 是 →
        - ✅ **该索引是否真的命中高频查询？**
            - 是 → ✅ 保留索引，**权衡读写性能**
            - 否 → ❌ 不建议建索引（写入代价太高）
    - 否 → ✅ 更加灵活，可适当增加索引提高查询效率

---

- ✅ **是否命中最左前缀原则？**
    - 是 → ✅ 可有效使用联合索引
    - 否 → ❌ 索引命中失败，建议调整字段顺序或拆分
